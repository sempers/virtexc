...
function submitRandomOrder(timeFactor, stock) {
    ...
    if (exchangeData.trades && exchangeData.trades.length > 0) {
        ...
        db.db
            ...
            function (err, trades) {
                if (err) {
                    io.sockets.emit('db_error',
                        "Database is probably full. You need to restart the exchange.");
                    exchangeReady = false;
                } else {
                    var data = exchangeData.stockData();
                    data.timestamp = trades[0]._id.getTimestamp().valueOf();
                    io.sockets.emit('exchangeData', data);
                    pauseThenTrade();
                }
            });
    } else
    ...
    function pauseThenTrade() {
        ...
        timeOutIds[stock] = setTimeout(submitRandomOrder.bind(this, timeFactor,
            stock), pause);
    }
}
...
function initAll(timeFactor, callback, restart) {
    ...
    if (restart)
        db.db.collection('transactions').remove({}, function (err) {
            endInit();
        });
    else
        endInit();
...
var createSocket = function (app) {
    ...
    io.sockets.on('connection', function (socket) {
        ...
        socket.on('requestData', function (data) {
            if (exchangeReady)
                socket.emit('initExchangeData', transformExchangeData(allData),
                    {
